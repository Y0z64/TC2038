...
/*
    * Implementación de Programación Dinámica y Programación Voraz para calcular
   el cambio
    * Nota: El programa espera que el archivo de texto tenga el formato
   correcto.
    *
    * Autores:
...
// Complejidad: O(C * N) en el peor caso, donde C es el tamano del cambio y N es
// el número de denominaciones Utilizando el Teorema Maestro: T(n) = aT(n/b) +
// f(n), donde a = 1, b > 1, f(n) = O(n) Caso 3 del Teorema Maestro: T(n) = Θ(n
// log n)
std::vector<int>
calculateChange(const std::vector<double> &denominations, double change,
                std::vector<int> &supply,
                HashTable<std::string, std::vector<int>> &cache) {
  // Redondear el cambio a 2 decimales para evitar problemas de precisión de
  // punto flotante
  ...
  for (int i = 0; i < N; ++i) {
    if (denominations[i] <= change && supply[i] > 0) {
      ...
      supply[i]--;
      std::vector<int> subResult = calculateChange(
          denominations, change - denominations[i], supply, cache);
  ...
  result[N - 1] =
      -1; // Usar el último elemento como bandera para solución inválida
  cache.insert(key, result);
  return result;
}
...
// Esta función utiliza un enfoque voraz (greedy) para encontrar una solución
// local
std::vector<int>
calculateChangeGreedy(const std::vector<double> &denominations, double change,
                      std::vector<int> &supply,
                      HashTable<std::string, std::vector<int>> &cache) {
  // Redondear el cambio a 2 decimales para evitar problemas de precisión de
  // punto flotante
  ...
  for (int i = 0; i < N && remainingChange > 1e-9; ++i) {
    ...
    int count = std::min(static_cast<int>(remainingChange / denominations[i]),
                         supply[i]);
    result[i] = count;
    remainingChange -= count * denominations[i];
    remainingChange = std::round(remainingChange * 100.0) /
                      100.0; // Redondear para evitar errores de punto flotante
  }

  // Si queda cambio por dar, la solución greedy no pudo encontrar una solución
  // válida
...
int main() {
  ...
  for (int i = 0; i < 5; ++i) {
    ...
    double executionTimeOptimal = measureExecutionTime([&]() {
      std::vector<int> result =
          calculateChange(denominations, change, supply, cache);
      ...
      if (i == 0) {
        ...
        if (result[N - 1] == -1) {
          ...
          for (int j = 0; j < N; j++) {
            if (result[j] > 0) {
              std::cout << result[j] << " x " << denominations[j] << " pesos"
                        << std::endl;
            }
          }
        }
    ...
    double executionTimeGreedy = measureExecutionTime([&]() {
      std::vector<int> result =
          calculateChangeGreedy(denominations, change, supply, cacheGreedy);
    ...
    inputSizes.push_back(change *
                         (i + 1)); // Simular un aumento en el tamaño de entrada

    std::cout << "Ejecución " << i + 1
              << " - Tiempo Óptimo: " << executionTimeOptimal
              << " ms, Tiempo Greedy: " << executionTimeGreedy << " ms"
              << std::endl;
  }
...
/* Despues de hacer varias pruebas en el codigo nos dimos cuenta de que:
    ...
    Aquí hay algunas diferencias clave entre los enfoques Greedy y de
   backtracking:

    1. Complejidad algorítmica:
    - El algoritmo Greedy tiene una complejidad de O(N).
    - El algoritmo de backtracking tiene una complejidad de O(C * N) en el peor
   caso.
    2. Enfoque de toma de decisiones:
    - Greedy toma decisiones localmente óptimas en cada paso, sin reconsiderar.
    - Backtracking explora múltiples combinaciones, lo que puede llevar más
   tiempo.

    3. Uso de memoria:
    - Greedy utiliza menos memoria, ya que no necesita almacenar estados
   intermedios.
    - Backtracking puede requerir más memoria debido a la recursión y
   almacenamiento de estados.

    4. Recorrido del árbol de decisiones:
    - Greedy sigue un único camino en el árbol de decisiones.
    - Backtracking explora múltiples ramas, lo que aumenta el tiempo de
   ejecución.

    5. Eficiencia en sistemas monetarios bien diseñados:
    - Greedy suele encontrar la solución óptima rápidamente.
    - Backtracking garantiza la optimizacion, pero a costa de explorar más
   posibilidades.

    6. Impacto de la caché:
    - Aunque ambos algoritmos usan caché, Greedy se beneficia más debido a su
   naturaleza.
    - Backtracking puede tener un patrón de acceso a caché menos predecible.

    7. Estructura de datos y operaciones:
    - Greedy utiliza principalmente operaciones simples y acceso secuencial a
   datos.
    - Backtracking involucra más operaciones complejas como la recursión.

    8. Pruning (poda) en backtracking:
    - Aunque el backtracking implementa poda, aún explora más posibilidades que
   Greedy.
    - Greedy naturalmente 'poda' al tomar decisiones inmediatas sin retroceder.

    9. Predictibilidad del flujo de ejecución:
    - El flujo de Greedy es más predecible, lo que puede llevar a mejor
   optimización por parte del compilador.
    - Backtracking tiene un flujo menos predecible debido a la naturaleza
   recursiva y las múltiples ramas.

    10. Localidad de referencia:
        - Greedy tiende a tener mejor localidad de referencia, aprovechando
   mejor la caché del procesador.
        - Backtracking puede sufrir más fallos de caché debido a su naturaleza
   recursiva y no lineal.

    Es importante notar que, aunque Greedy es generalmente más rápido, no
   siempre garantiza la solución óptima en todos los casos posibles,
   especialmente en sistemas con restricciones complejas.
    */